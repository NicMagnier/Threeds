<?php
	...
	if ( $Threeds->Auth->isLazyRegistration()==false ) {
		header("Location: /");
	}
	...
	if ( !empty($_POST['login']) ) {
		...
		else if ( $Threeds->Auth->isValidEmail( $_POST['login'])==false ) {
			$message = "<p>This is not a valid e-mail address.</p>";
		}
		else if ( $Threeds->secureLazyAccount( $_POST['login'], $_POST['pass1']) === true ) {
			header("Location: /u");
		}
		else {
...
?>
	<div class="form500"><div class="formBlock">
		<h2>Connect with a Twitter account</h2>
		<a href="/twitter/signin"><input type="button" value="Signin with Twitter..."/></a>
	</div></div>
	
	<form class="form500" method="post" action="/secure">
	<div class="formBlock">
		...
		<p><input type="submit" name="submit" value="Secure your account" /></p>
	</div>
	</form>
